...
/**
 * 处理在使用 vite-plugin-optimizer 之后，
 * 自定义的路径别名无法使用的问题
 */
export const aliasPlugin = (): PluginOption => {
  return {
    name: 'alias-plugin',
    config(config: UserConfig) {
      if (config.resolve?.alias instanceof Array) {
        config.resolve.alias = config.resolve.alias.map((item) => {
          if (typeof item.replacement === 'object') {
...
export const getReplacer = (): Entries => {
